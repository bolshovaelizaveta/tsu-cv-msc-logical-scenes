...
class SemanticSceneAnalyzer:
    ...
    def analyze_shots(self, shots_dir):
        ...
        for filename in tqdm(shot_files, desc="Получение эмбеддингов"):
            ...
            if embedding is not None: shot_embeddings[shot_number] = embedding
        if len(shot_embeddings) < 2: return np.array([])
        ...
        if len(upper_triangle_indices[0]) == 0: return np.array([])
        unique_distances = distances_matrix[upper_triangle_indices]
        distance_threshold = np.percentile(unique_distances, PERCENTILE)
        clustering = AgglomerativeClustering(n_clusters=None, distance_threshold=distance_threshold, linkage='complete', metric='cosine')
        cluster_labels = clustering.fit_predict(all_embeddings)

        # UPD:вместо поиска готовых сцен, теперь вычисляем массив вероятностей для каждой границы
        break_probabilities = []
        WINDOW_SIZE = 2 # Используем маленькое окно для вычисления вероятности на конкретной границе
        
        for i in range(len(cluster_labels) - 1): # Проходим по всем границам
